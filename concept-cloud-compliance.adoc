...
* Condivisioni di file (ad esempio, Dell EMC Isilon, pure Storage, Nutanix e così via)
* Database:
+
...
La classificazione BlueXP è ora gratuita da utilizzare per un massimo di 500 TiB di dati. Non è richiesta alcuna licenza di classificazione o abbonamento a pagamento. Se si desidera eseguire la scansione di più di 500 TiB di dati, è possibile disattivare la scansione su alcuni dei sistemi originali ed eseguire la scansione dei nuovi sistemi che altrimenti porterebbero al di sopra del limite di 500 TiB.
