...
summary: Completare alcuni passaggi per installare la classificazione BlueXP su un host Linux in un sito on-premise che non dispone di accesso a Internet. Questo tipo di installazione è perfetto per i siti sicuri. 
---
...
[role="lead"]
Completare alcuni passaggi per installare la classificazione BlueXP su un host Linux in un sito on-premise che non dispone di accesso a Internet. Questo tipo di installazione è perfetto per i siti sicuri.
...
Una volta installato in questo modo (a volte chiamato sito "offline" o "buio"), la classificazione BlueXP può eseguire la scansione dei dati solo da origini dati che sono anche locali al sito on-premise. Al momento, la classificazione BlueXP è in grado di eseguire la scansione delle seguenti origini dati *locali*:
...
Attualmente non è disponibile alcun supporto per la scansione di Cloud Volumes ONTAP, Azure NetApp Files, FSX per ONTAP, AWS S3 o Google Drive, OneDrive o SharePoint Online quando la classificazione BlueXP viene implementata in un sito buio.
...
Se non si dispone già di un connettore installato sul sito on-premise offline, https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-private-mode.html["Implementare il connettore"^] Su un host Linux.
...
Se non si dispone già di un connettore BlueXP installato sul sito on-premise offline, https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-private-mode.html["Implementare il connettore"^] Su un host Linux nel tuo sito offline.
...
. Copiare il pacchetto di installazione sull'host Linux che si intende utilizzare nel sito dark.
...
.Prima di iniziare
* Il software di classificazione BlueXP può essere aggiornato una versione principale alla volta. Ad esempio, se è installata la versione 1.20.x, è possibile eseguire l'aggiornamento solo alla versione 1.21.x. Se si dispone di alcune versioni principali, sarà necessario aggiornare il software più volte.
* Verificare che il software BlueXP Connector sia stato aggiornato alla versione più recente disponibile. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-managing-connectors.html#upgrade-the-connector-when-using-private-mode["Consultare la procedura di aggiornamento del connettore"^].
